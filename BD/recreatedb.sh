...
createdb cican -O cican -E utf8 && psql cican -U cican < tablas.sql 2>&1 | grep -v NOTICE
if [ $DEBUG -eq 1 ]; then
    echo Creando usuarios... 
fi 
psql cican < usuarios.sql
if [ $DEBUG -eq 1 ]; then
...
psql cican < paises.sql
if [ $DEBUG -eq 1 ]; then
    echo Insertando provincias...
fi 
psql cican < provincias.sql
if [ $DEBUG -eq 1 ]; then
    echo Insertando municipios...
fi 
psql cican < municipios.sql
if [ $DEBUG -eq 1 ]; then
    echo Insertando tipos de vía...
fi 
psql cican < tipos_de_via.sql
if [ $DEBUG -eq 1 ]; then
    echo Insertando categorías laborales...
fi 
psql cican < categorias_laborales.sql
if [ $DEBUG -eq 1 ]; then
    echo Insertando formas de pago...
fi 
psql cican < formas_de_pago.sql
if [ $DEBUG -eq 1 ]; then
    echo Insertando tabla de medidas...
fi 
psql cican < unidades.sql
if [ $DEBUG -eq 1 ]; then
