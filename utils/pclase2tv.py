...
import sys, os
diractual = os.path.basename(os.path.realpath(os.path.curdir)) 
dirfichero = os.path.basename(os.path.dirname(os.path.realpath(__file__))) 
if diractual == dirfichero:
    os.chdir("..")
sys.path.append(".")
...
def edit_fecha(cell, path, newtext, tv, numcol, clase, atributo):
    ...
    objeto = clase.get(id)
    try:
        fecha = utils.fecha.parse_fecha(newtext)
...
def edit_fechahora(cell, path, newtext, tv, numcol, clase, atributo):
    ...
    objeto = clase.get(id)
    try:
        fecha = utils.fecha.parse_fechahora(newtext)
    except (ValueError, TypeError):
        parent = tv.parent
        while parent != None:
            parent = parent.parent
...
class Pclase2tv:
    ...
    def __init__(self, pclase, tv, objeto_clave_ajena = None, 
        ...
        cols = self.crear_cols(nombres_col)
        utils.ui.preparar_listview(self.tv, cols, multi = seleccion_multiple)
    ...
    def crear_cols(self, nombres_col = []):
        """
        ...
        cols = []
        numcol = 0
        for col in self.clase.sqlmeta.columnList:
