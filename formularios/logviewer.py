...
def buscar_fichero_log():
    rutas_posibles = (".", "..", os.path.join("..", "formularios"))
    for subruta_relativa in rutas_posibles:
        fichero_log = os.path.join(os.path.dirname(__file__), 
                                   subruta_relativa, "ginn.log")
        if os.path.exists(fichero_log):
            break
    return fichero_log 
...
""", 
    ...
    def agregar_linea(self, model, linea):
        ...
        """
        fecha = utils.fecha.str_fecha(linea[0])
        hora = utils.fecha.str_hora(linea[1])
        tipo = linea[2]
        usuario = linea[3]
        texto = linea[4]
        numlinea = linea[5]
    ...
    def obtener_hora(self, linea, fecha, hora_anterior):
        ...
        """
        # No hay Epoch en datetime ni en time 
        hora = datetime.datetime(1, 1, 1)
        try:
            ...
            mes = fecha.month
            anno = fecha.year
            hora = datetime.datetime(day = dia, month = mes, year = anno, hour = horas, minute = minutos, second = segundos)
        except:
    ...
    def obtener_fecha(self, linea, fecha_anterior):
        ...
        try:
            ...
            fecha = datetime.date(day = dia, month = mes, year = anno)
        except:
