...
def get_IPLocal():
    ...
    ifaces = ("eth0", "wlan0", "eth3", "wlan3", "ra0")
    for iface in ifaces:
        try:
...
class Autenticacion(Ventana):
    ...
    def loginfailed(self):
        ...
        self.wids['e_passwd'].grab_focus()
        imagen_llave = os.path.join('imagenes', 'llave.png')
        if not os.path.exists(imagen_llave):
            imagen_llave = os.path.join("imagenes", "llave.png")
        self.wids['image1'].set_from_file(imagen_llave)
    ...
    def do_login(self, usuario, passwd):
        ...
        ok = user.count() == 1
        if user.count() > 1:
            self.logger.error("Caso imposible. Más de un usuario con el mismo"
                              " nombre de usuario: %s "
                              "¡Constraint de la BD falló!" % (usuario))
        if ok:
