...
# Versión multiplataforma del startfile de Windows y que hasta el momento no 
# había conseguido emular bien del todo en GNU/Linux.
# Encontrado en http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/511443.
# No aclara licencia y el autor (Antonio Valentino) no facilita email, supongo 
# que puedo redistribuirlo con el resto de mi código GPL. En cualquier caso, 
...
if sys.platform[:3] == 'win':
    ...
    class Start(BaseController):
        ...
        def open(self, filename):
            try:
                os.startfile(filename)  # Solo Windows
            except WindowsError:        # Solo Windows
    ...
    #  File "Q:\formularios\facturas_compra.py", line 1948, in ver_adjunto
    #    from multi_open import open as mopen
    #  File "Q:\formularios\multi_open.py", line 115, in <module>
    #    _controllers['windows-default'] = Start()
    # TypeError: __init__() takes exactly 2 arguments (1 given)
    _controllers['windows-default'] = Start("startfile")
    ...
    for headername in ('to', 'cc', 'bcc', 'subject', 'body', 'attach'):
        if kwargs.has_key(headername):
            ...
            if headername in ('address', 'to', 'cc', 'bcc'):
                ...
                headervalue = encode_rfc2231(headervalue) # check
                parts.append('%s=%s' % (headername, headervalue))
