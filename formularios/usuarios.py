...
class Usuarios(VentanaGenerica):
    ...
    def es_diferente(self):
        ...
        if not condicion and pclases.DEBUG:
            print "usuaruos.py::es_diferente", "sp_nivel", nivel_ventana, \
                                                           nivel_usuario
        condicion = (condicion and 
    ...
    def inicializar_ventana(self, *args, **kw):
        """
        ...
        utils.ui.preparar_treeview(self.wids['tv_permisos'], cols)
    ...
    def preparar_tab_modulos(self):
        cols = (('Módulo/Ventanas', 'gobject.TYPE_STRING', 
                 True, True, True, self.cambiar_nombre_modulo),
                ('Descripción', 'gobject.TYPE_STRING', 
                 True, True, False, self.cambiar_descripcion_modulo),
                ('Icono', 'gobject.TYPE_STRING', 
                 True, True, False, self.cambiar_icono_modulo),
                ('ID', 'gobject.TYPE_INT64', False, False, False, None))
        utils.ui.preparar_treeview(self.wids['tv_modulos'], cols)
        ...
        cols = (('Descripción', 'gobject.TYPE_STRING', 
                 True, True, True, self.cambiar_descripcion_ventana),
                ('Fichero', 'gobject.TYPE_STRING', 
                 True, True, False, self.cambiar_fichero_ventana),
                ('Clase', 'gobject.TYPE_STRING', 
                 True, True, False, self.cambiar_clase_ventana),
                ('Icono', 'gobject.TYPE_STRING', 
                 True, True, False, self.cambiar_icono_ventana),
                ('ID', 'gobject.TYPE_INT64', False, False, False, None))
        utils.ui.preparar_listview(self.wids['tv_ventanas'], cols)
    ...
    def rellenar_tab_modulos(self):
        ventanas = [(v.id, v.descripcion) 
                    for v in pclases.Ventana.select(orderBy = "descripcion")]
        utils.ui.rellenar_lista(self.wids['cb_add_ventana'], ventanas)
    ...
    def rellenar_modulos(self):
        ...
        for m in pclases.Modulo.select():
            ...
            for v in m.ventanas:
                try:
                    fileicono = os.path.join('imagenes', v.icono)
                    pixbuf = gtk.gdk.pixbuf_new_from_file(fileicono)
                except (gobject.GError, AttributeError, TypeError):
                    fileicono = os.path.join('imagenes', 'dorsia.png')
                    pixbuf = gtk.gdk.pixbuf_new_from_file(fileicono)
    ...
    def ir_a_primero(self):
        ...
        try:
            ...
            usuario = pclases.Usuario.select(orderBy = 'id')[0] 
                # Selecciono todos los usuarioes de venta y me quedo con el 
                # primero de la lista.
            usuario.notificador.activar(self.aviso_actualizacion)   # Activo 
                                                            # la notificación
        except:
            usuario = None  
        self.objeto = usuario
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        idusuario = utils.ui.dialogo_resultado(filas_res,
                        ...
                        cabeceras = ('ID', 'Usuario', "Nombre", 
                                     "Cuenta soporte", "Nivel de privilegios"),
                        padre = self.wids['ventana']) 
        if idusuario < 0:
            return None
        else:
    ...
    def rellenar_widgets(self):
        ...
        self.wids['ventana'].set_title(
            "Gestión de usuarios - %s" % (usuario.usuario))
    ...
    def rellenar_permisos(self):
        ...
        model.clear()
        for m in pclases.Modulo.select():
            ventanasconpermiso = [p.ventana 
                                  for p in usuario.permisos 
                                  if p.permiso and p.ventana.modulo == m]
            if (len(ventanasconpermiso) == len(m.ventanas) 
                and len(m.ventanas) > 0):
                permisomodulo = True
            else:
                permisomodulo = False
            if (len([p.ventana 
                     for p in usuario.permisos 
                     if p.lectura and p.ventana.modulo == m]) 
                == len(m.ventanas) and len(m.ventanas) > 0):
                rmodulo = True
            else:
                rmodulo = False
            if (len([p.ventana 
                     for p in usuario.permisos 
                     if p.escritura and p.ventana.modulo == m]) 
                == len(m.ventanas) and len(m.ventanas) > 0):
                wmodulo = True
            else:
                wmodulo = False
            if (len([p.ventana 
                     for p in usuario.permisos 
                     if p.nuevo and p.ventana.modulo == m]) 
                == len(m.ventanas) and len(m.ventanas) > 0):
                xmodulo = True
            else:
                xmodulo = False
            iterpadre = model.append(None, (m.nombre, 
                                            ...
                                            xmodulo,
                                            m.id))
            # PLAN: El CellRendererToggle tiene una propiedad "inconsistent". 
            # Se podría usar para indicar si tiene permisos sobre algunas de 
            # las ventanas del módulo.
            for v in m.ventanas:
                model.append(iterpadre, 
                             (v.descripcion,
                              v.id in [p.ventana.id 
                                       for p in usuario.permisos 
                                       if p.permiso],
                              v.id in [p.ventana.id 
                                       for p in usuario.permisos 
    ...
    def set_permiso(self, ventana, permitir, r = None, w = None, x = None):
        ...
        if ventana not in [p.ventana for p in usuario.permisos]:
            p = pclases.Permiso(usuario = usuario, ventana = ventana, 
                                permiso = permitir)
        else:
            p = [p for p in usuario.permisos if p.ventana == ventana][0]
            p.permiso = permitir
        if r != None:
            p.lectura = r or permitir # Si se le permite abrir lo lógico es 
                            # que pueda leer registros ya existentes al menos.
        if w != None:
            p.escritura = w
        if x != None:
            p.nuevo = x
    ...
    def enviar_mensaje(self, b):
        usuario = self.objeto
        texto = utils.ui.dialogo_entrada(titulo = 'NUEVO MENSAJE', 
                                texto = 'Introduzca mensaje de la alerta:', 
                                padre = self.wids['ventana'])
        if texto != None:
    ...
    def borrar_mensaje(self, b):
        selection = self.wids['tv_mensajes'].get_selection()
        model, paths = selection.get_selected_rows()
        for path in paths:
            aid = model[path][-1]
            alerta = pclases.Alerta.get(aid)
            if not alerta.entregado:
                utils.ui.dialogo_info(titulo = "ALERTA PENDIENTE", 
                    texto = "La alerta seleccionada aún no ha sido leída "
                            "por el usuario.\nMárquela como leída si "
                            "realmente quiere borrarla.", 
                    padre = self.wids['ventana'])
            else:
                try:
                    alerta.destroySelf()
                    ...
                    utils.ui.dialogo_info(titulo = "ERROR", 
                        texto = "El mensaje no se pudo borrar", 
                        padre = self.wids['ventana'])
        self.rellenar_alertas()
    ...
    def cambiar_fechahora(self, cell, path, text):
        ...
        try:
            ...
            alerta.fechahora = mx.DateTime.DateTimeFrom(day = dia, 
                                                        month = mes, 
                                                        year = anno, 
                                                        hour = horas, 
                                                        minute = minutos)
        except:
            utils.ui.dialogo_info(titulo = 'HORA INCORRECTA', 
                    texto = 'El formato debe ser "dia/mes/año horas:minutos"', 
                    padre = self.wids['ventana'])
        self.rellenar_alertas()
    ...
    def crear_nuevo_usuario(self, widget):
        ...
        try:
            ...
            # Seguramente haya introducido un nombre repetido o en blanco 
            # (valor por defecto del diálogo).
            utils.ui.dialogo_info(padre = self.wids['ventana'])
            return
        ...
        usuario = pclases.Usuario(usuario = nomusuario,
                                        ...
                                        cuenta = '',
                                        cpass = '')
        utils.ui.dialogo_info('USUARIO CREADO', 
            'El usuario %s ha sido creado.\nNo olvide completar el resto de '
            'información relativa al mismo.' % usuario.usuario, 
            padre = self.wids['ventana'])
        usuario.notificador.activar(self.aviso_actualizacion)
        self.objeto = usuario
        self.actualizar_ventana()
    ...
    def buscar_usuario(self, widget):
        ...
        a_buscar = utils.ui.dialogo_entrada(
            "Introduzca nombre de usuario o nombre real:", 
            padre = self.wids['ventana']) 
        if a_buscar != None:
            resultados = pclases.Usuario.select(
                sqlobject.OR(pclases.Usuario.q.usuario.contains(a_buscar),
            ...
            if resultados.count() > 1:
                ...
                utils.ui.dialogo_info('SIN RESULTADOS', 
    ...
    def guardar(self, widget):
        ...
        if usuario.firmaTotal:
            usuario.firmaComercial = usuario.firmaDirector \
                = usuario.firmaTecnico = usuario.firmaUsuario = True
        buffer = self.wids['txt_observaciones'].get_buffer()
        usuario.observaciones = buffer.get_text(buffer.get_start_iter(), 
                                                buffer.get_end_iter())
    ...
    def nueva_contrasenna(self, b):
        ...
        try:
            ...
            utils.ui.dialogo_info(titulo = 'CONTRASEÑA CAMBIADA', 
    ...
    def add_ventana_a_modulo(self, b):
        ...
        if iter == None or model[iter].parent != None:
            utils.ui.dialogo_info(titulo = 'SELECCIONE MÓDULO', 
                texto = 'Debe seleccionar un módulo al que añadir la ventana.',
                padre = self.wids['ventana'])
            return
        idventana = utils.ui.combo_get_value(self.wids['cb_add_ventana'])
        if idventana == None:
            utils.ui.dialogo_info(titulo = 'SELECCIONE VENTANA', 
                texto = 'Debe seleccionar una ventana para añadir al módulo.', 
                padre = self.wids['ventana'])
    ...
    def add_modulo(self, b):
        ...
        if nombre:
            m = pclases.Modulo(nombre = nombre, icono = icono, 
    ...
    def add_ventana(self, b):
        ...
        if fichero and descripcion and clase:
            v = pclases.Ventana(descripcion = descripcion, icono = icono, 
                                clase = clase, fichero = fichero, 
                                modulo = None)
    ...
    def borrar_usuario(self, boton):
        ...
        él y sus LDVs.
        """
        if not utils.ui.dialogo('Se intentará eliminar el usuario actual '
                'y todos sus permisos y datos relacionados.\n¿Está seguro?', 
                'BORRAR USUARIO', 
                padre = self.wids['ventana']): 
            return
        usuario = self.objeto
