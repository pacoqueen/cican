...
class MainWindow(gtk.Window):
    ...
    def confirm_clicked(self, button, location = None, lat = None, lon = None):
        # XXX: By Queen.
        # Permite recibir la cadena del sitio a buscar. Si no se especifica, 
        # entonces usa lo que haya escrito en el combo. 
        if lat == lon == None:
            if not location: 
        ...
        if -180 <= longitude <= 180 and -90 <= latitude <= 90:
            ...
            if (not location in locations.keys()):
                if self.cb_offline.get_active():
                    if error_msg(self, _("Offline mode, cannot do search!"
    ...
    def on_cb_operations_changed(self, cb_operations):
        """combo box dispatches operation and returns to default 
        position - Operations - 1st item"""
           ...
           (self.conf.status_location == STATUS_GPS and not mapGPS.available)):
            ...
            if (self.Ruler):
                ...
                if (self.segment_no == -1): # Before First Click
                    self.status_bar.push(self.status_bar_id, "Ruler Mode - "
                ...
                if (self.segment_no > 0):#Mouse-motion:There is a line to draw
                    ...
                    da.draw_line(gc, self.ruler_coordx[so], 
                                 self.ruler_coordy[so], x, y, "", zl)
                    screen_coord1 = self.drawing_area.coord_to_screen(
                                                      self.ruler_coordx[so], 
                                                      self.ruler_coordy[so], 
                                                      self.ruler_coordzl[so])
                    screen_coord2 = self.drawing_area.coord_to_screen(x, y, zl)
                    ...
                    if (z > 10):
                        self.status_bar.push(self.status_bar_id, "New Segment" 
                            " Distance = %.4f km, Total distance = %.4f km" % (
                              z, self.total_dist))
                    else:
                        self.status_bar.push(self.status_bar_id, "New Segment"
                            " Distance = %.2f m, Total distance = %.4f km" % (
                              (z * 1000), self.total_dist))            
            else:
                self.status_bar.push(self.status_bar_id, "Latitude=%.6f "
                                     "Longitude=%.6f" % (coord[0], coord[1]))
    ...
    def da_button_press(self, w, event):
        ...
        if (event.button == 1):
            coord_x = ((w.center[0][0]) * TILES_WIDTH + w.center[1][0] 
                        - w.allocation.width/2 + event.x)
            coord_y = ((w.center[0][1]) * TILES_HEIGHT + w.center[1][1] 
                       - w.allocation.height/2 + event.y)
            ...
            log.debug("Selected tile: [%d.%d, %d.%d] - lat/lon: [%s,%s]" % (
                      tile[0][0],tile[1][0],tile[0][1],tile[1][1], coords[0], 
                      coords[1]))
    ...
    def __init__(self, parent=None, config_path=None):
        ...
        self.container = vbox
        # XXX: By Queen. Comento todo porque no quiero que se construya la 
        #      ventana. Solo quiero el contenedor de mapa y controles.
        ...
        # XXX: By Queen
        self.top_panel.set_property("visible", False)
        # self.left_panel.set_property("visible", False)
        # self.status_bar.set_property("visible", False)
        self.cb_offline.set_active(False)
        #self.confirm_clicked(None, location = "Pagés del Corro, 182. Sevilla.")
        # XXX
