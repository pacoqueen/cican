...
def build_tabla_contenido(data):
    ...
    for d in data:
        fila = (escribe(d[0]),
                ...
                Paragraph(escribe(
                    numero.float2str(d[2])), estilo_numeros_tabla),
                Paragraph(escribe(
                    numero.float2str(d[3])),estilo_numeros_tabla),
                Paragraph(escribe(numero.float2str(d[2] * d[3])),
...
def build_tabla_totales(totales, observaciones = [], vencimientos = []):
    """
    ...
    «vencimientos» es la lista de vencimientos para estimar cuánto ocupará y 
    subir en proporción la tabla de totales.
    """
    ...
    # XXX Todo esto es estimativo. Se podría afinar más usando la altura de la 
    # fuente y calculando el número de líneas que ocupará el texto, pero eso 
    # requeriría un tiempo de computación extra que no estoy dispuesto a 
    # asumir. Ya tarda bastante la factura de por sí como para meterle más 
    # cálculos por apenas unos milímetros de precisión:
    h = (3*cm       # Altura inicial de tabla de vencimientos.
         + 2.5*cm   # Altura de la caja de totales.
         + 0.25*cm) # Altura de la caja gris con los textos de los totales.
    num_vtos_extra = max(len(vencimientos) - 2, 0)
    hvtos = num_vtos_extra * 1.1 * cm   # Dos vencimientos es 
        # lo que cabe aproximadamente en la caja inicial.
    if isinstance(observaciones, str):
        observaciones = [observaciones]
    lineas_observaciones = sum([(len(o)/52)+1 for o in observaciones[2:]])
    # Ya contaba con las dos líneas de IMPORTA LA PRESENTE FACTURA. 
    # Y 52 letras por línea por poner algo... 
    hobs = 0.65 *cm * lineas_observaciones 
    if pclases.DEBUG:
        print >> sys.stderr, lineas_observaciones, hvtos, hobs
    h += max(hvtos, hobs)
    # XXX
    tabla = TablaFija(78,
                      h,
...
def go(titulo,
    ...
    tabla_totales = build_tabla_totales(totales, observaciones, vencimientos)
...
def go_from_facturaVenta(facturaVenta):
    ...
    sueltos.
    """
    obra = facturaVenta.obra
    cliente = obra.cliente
    direccion = obra.direccion
    if not direccion:
        direccion=obra.direccion=pclases.Direccion.get_direccion_por_defecto()
    ciudad = direccion.ciudad
    provincia = ciudad.provincia
    datos_cliente = {"código": cliente.codigo or "",
                     "cif": cliente.cif or "",
                     "razón social": obra.nombre,
                     "dirección": direccion.get_direccion_completa(),
                     "población": ciudad and ciudad.ciudad or "",
                     "provincia": provincia and provincia.provincia or ""}
    if (direccion.codigoPostal and direccion.codigoPostal.cp 
        and direccion.codigoPostal.cp.strip()):
        datos_cliente["población"] = (direccion.codigoPostal.cp + " - "
                                      + datos_cliente["población"])
    datos_factura = {"fecha": fecha.str_fecha(facturaVenta.fecha),
                     "número": facturaVenta.numfactura,
                     "codcliente": cliente.codigo and cliente.codigo or ""} 
    iva = cliente.iva
    # TODO: ¿Usar el float2str_autoprecision?
    lineas_contenido = [(ldv.producto and ldv.producto.codigo or "",
                         ldv.get_descripcion_completa(), 
                         ldv.cantidad,
                         ldv.calcular_total())
    ...
    vencimientos = []
    forma_de_pago = []
    vtos = [v for v in facturaVenta.vencimientosCobro if not v.es_retencion()]
    vtos.sort(lambda v1, v2: (v1.fecha < v2.fecha and 1)
                              or (v1.fecha > v2.fecha and -1)
                              or 0)
    for v in vtos:
        strvto = "{0} ({1:} €)".format(fecha.str_fecha(v.fecha),
                                      numero.float2str(v.importe))
        vencimientos.append(strvto)
    if facturaVenta.retencion:
        vto_retencion = facturaVenta.get_vencimiento_retencion()
        if vto_retencion:
            vencimientos.append("{0.observaciones}: {0.importe:.2f} €".format(
                vto_retencion)) 
    try:
        forma_de_pago = facturaVenta.formaDePago.get_info()
    ...
    return go("Factura de venta %s (%s)" % (
                facturaVenta.obra.nombre,
                fecha.str_fecha(facturaVenta.fecha)),
              nomarchivo,
              datos_cliente,
              datos_factura,
              lineas_contenido,
